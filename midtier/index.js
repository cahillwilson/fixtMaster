...
router.get('/sandbox', function (req, res, next) {
   logger.info('get all sandboxes [user ' + req.headers['userid'] + ']');
    var sandboxes = [];
    models.Sandboxes.find({'userId': req.headers['userid']}, function(err, sandboxes) { 
        var leaves = traverse(JSON.stringify(sandboxes[0])).reduce(function (acc, x) {
...
router.post('/sandbox/:sandboxId/:nodeId', function (req, res) {
    ...
    models.Sandboxes.findByIdAndUpdate(
        ...
        {$push: {cards: card}, $set: {timestamp: timenow}},
...
router.delete('/card/:cardId', function(req, res) {
    ...
    models.Cards.remove({_id: req.params.cardId}, function (err) {
       if (err) {
...
router.delete('/sandbox/:sandboxId', function(req, res, next) {
    ...
    models.Sandboxes.remove({_id: req.params.sandboxId}, function(err) {
        if (err) {
...
router.get('/initialSearch/:searchCategory/:searchType/:searchString', function(req, res) {
    ...
    request({
        url: 'http://localhost:9000/services/BGWFIXT/v1/search/initialSearch/' 
            + req.params.searchCategory + '/'
            + req.params.searchType + '/'
            + req.params.searchString,
        qs: {start: req.query.start, end: req.query.end, segment: req.query.segment},
        method: 'GET',
        headers: {
            'TransactionID': '1234',
            'UserID': req.headers['userid']
        ...
        if (error) {
            ...
            models.Users.findOne({'userId': req.headers['userid']}, function(err, user) {
                if (err) {
                    ...
                    models.Roles.findOne({'index': user.role}, function(err, role) {
...
function prepareNodeForCard(body, fromCache, res) {
    models.Users.findOne({'userId': userId}, function(err, user) {
        if (err) {
            logger.info('db lookup error for user');
            res.send('Lookup error');
        } else if (user === null) {
...
function findNode(accountNumber, hierarchyPointId, userId, res) {
      logger.info('finding node in CSIBGW');
    request({
        url: 'http://localhost:9000/services/BGWFIXT/v1/hierarchy/invoiceNode',
        qs: {accountNumber: accountNumber, hierarchyPointId: hierarchyPointId},
        method: 'GET',
        headers: {
            'TransactionID': '1234',
            'UserID': userId
